...
import (
	...
	"github.com/assembly-hub/gauss"
	"github.com/assembly-hub/orm"
	"github.com/assembly-hub/orm/dbtype"

	"github.com/assembly-hub/orm-example/gauss/dao"
)

var dbRef = orm.NewReference(dbtype.OpenGauss)
...
// 获取数据库连接
func getDB() *sql.DB {
	cli := gauss.NewClient(&gauss.Config{
		User:     "gaussdb",
		Password: "Enmo@123",
		Host:     "localhost",
		Port:     5433,
...
func insertData(tb *orm.ORM) {
	...
	_, _, err := tb.InsertMany([]interface{}{dt}, true)
	if err != nil {
		fmt.Println(err)
	}
}
