...
import (
	...
	"github.com/assembly-hub/orm-example/postgres/dao"
)

var dbRef = orm.NewReference(dbtype.Postgres)
...
// 获取数据库连接
func getDB() *sql.DB {
	cli := postgres.NewClient(&postgres.Config{
		User:     "postgres",
		Password: "123456",
		Host:     "localhost",
		Port:     5432,
...
func insertData(tb *orm.ORM) {
	...
	_, _, err := tb.InsertMany([]interface{}{dt}, false)
	if err != nil {
		fmt.Println(err)
	}
}
