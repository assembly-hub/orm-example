...
import (
	...
	"github.com/assembly-hub/mariadb"
	"github.com/assembly-hub/orm"
	"github.com/assembly-hub/orm/dbtype"

	"github.com/assembly-hub/orm-example/mariadb/dao"
)

var dbRef = orm.NewReference(dbtype.MariaDB)
...
// 获取数据库连接
func getDB() *sql.DB {
	cli := mariadb.NewClient(&mariadb.Config{
		Host:            "127.0.0.1",
		Port:            3307,
		Username:        "root",
		Password:        "1aA",
...
func query(tb *orm.ORM) {
	...
	*/
	tb.Query("id__gt", 1, "name__startswith", "test")
	// 等价
	tb.Wheres(orm.Where{
