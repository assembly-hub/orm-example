...
import (
	...
	"github.com/assembly-hub/orm-example/sqlite/dao"
)

var dbRef = orm.NewReference(dbtype.SQLite3)
...
// 获取数据库连接
func getDB() *sql.DB {
	cli := sqlite.NewClient(&sqlite.Config{
		//Username:        "root",
		//Password:        "root",
		DBName: "/mnt/f/sqlite/sqlite3.db",
	})
	db, err := cli.Connect()
	if err != nil {
		panic(err)
